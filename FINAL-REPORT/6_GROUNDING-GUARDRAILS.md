...
## 6.3 ② grounding — 유효숫자 대조로 환각 탈락

`grounding.py`가 가장 핵심적인 장치다. 관점 의심건의 인용 수치가 실데이터에 있는지 **유효숫자 동일성**으로 대조한다. `_sig`는 숫자만 추출한 뒤 앞뒤 0을 제거해(`grounding.py:31-35`) 스케일·단위 무관하게 비교한다 — 원·백만·억·조 어느 표기든, float 직접 비교를 회피한다.

인덱스는 series_key/canonical/label/account_id를 유효숫자 집합으로 색인하며, **sj_div 한정 키도 색인**해(동명이계 오매칭 차단, verify가 우선 조회) 손익계산서와 현금흐름표의 같은 계정명·다른 금액을 구분한다. 주석 fact는 `note:{label}`/`note:{category}` 네임스페이스로 색인하고, 서술형 공시(담보·특수관계자)는 token별 + 전역 `__disclosure__` 풀로 색인한다(사각#3 해결).
