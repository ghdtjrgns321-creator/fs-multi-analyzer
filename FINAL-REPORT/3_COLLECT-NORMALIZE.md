...
> **위치**: `[L0 수집] → [L1 정규화] → [온보딩 게이트]` → LLM 전처리 → L2 신호엔진. 이 세 레이어에는 **LLM 호출이 전혀 없다** — 전부 결정론 코드(수집·매핑·검산)이며, 게이트는 UI [분석 실행]이 정규화 직후 자동 수행한다. LLM은 게이트 통과 뒤의 LLM 전처리(별칭 제안 — 코드가 좁힌 후보 안에서만 — 과 사업보고서 본문 읽기)에서만 관여한다. G6 dump는 점검용 산출로 계속 생성되지만 이를 읽는 LLM은 배선돼 있지 않다.
...
사업보고서 XML          map_change_row (SCE label-first)   G5 신호 무결성 + G8 번역 품질
                       ...
                       _dedupe (소실 대신 강등 보존)          G6 dump(LLM 읽기 입력)
...
회사마다 라벨·확장계정이 무한 변주하므로, 처음 보는 회사는 정규화 직후 게이트를 거쳐야 L2에 진입한다. 이 검문은 UI [분석 실행]이 자동 수행하며(통과한 연도만 `prep.json` 마커를 받는다), 별칭 3단 분업은 게이트와 별개로 LLM 전처리 단계에서 돈다. 게이트는 기존 감사 스크립트를 **재사용**(재구현 금지)한다.
...
LLM 전처리는 별칭 자동 등록과 함께 사업보고서 본문을 파트별로 읽어 서술형 감사관심을 적재한다(주석 관점의 입력). 완료 마커가 카드 단계 진입 조건이며, 사람이 직접 등록하는 폼은 별도 정비 페이지에만 남아 있다(7장 §7.5).
