...
```
  ...
  (~12만 회사)          + 주석 XBRL + 원문XML       2,015 표준계정          ("분석 준비"가 자동 실행)
                                                  회사/연도 격리 DuckDB
                                                                              │ 통과
                                                                              ▼
     [LLM 전처리]  별칭 3단 분업(코드 후보→LLM 선택→신뢰도≥0.7만 자동 등록) + 본문 통독 서술추출 — 완료 마커가 카드 단계 진입 조건
                                                                              │
                                                                              ▼
  ┌───────────────────────── L2 신호엔진 (결정론, LLM 0) ─────────────────────────┐
  │  전수 스캔 universal · 다축 프로파일러(self 4축) · 관계사슬 9개 · 비율 15개     │
...
| 골든 검사1(수치)                       | LG생활건강 **N=72 전량 match**, 아스트 **N=53 전량 match** | [9장](9_GOLDEN-TESTS-DECISIONS.md) |
