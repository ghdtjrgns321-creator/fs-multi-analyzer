...
> **한 줄 요약** — OpenDART 공시 재무제표·주석을 결정론 코드로 전량 계산하고, **발견만** 6개 관점 LLM에 맡겨 교차검증한 뒤, 감사인이 검토할 **리스크 후보 카드**를 근거·반박·다음절차와 함께 제시하는 Human-in-the-Loop 리뷰 도구다. **부정을 확정하지 않는다.**
...
```
  ...
  ─────────      →      finstate JSON            XBRL→canonical    →   G1~G6 결정론 검문
  (~12만 회사)          + 주석 XBRL + 원문XML       2,015 표준계정          + 별칭 3단 분업
                                                  회사/연도 격리 DuckDB     (코드→LLM→사람)
                                                                              │ 통과
                                                                              ▼
  ┌───────────────────────── L2 신호엔진 (결정론, LLM 0) ─────────────────────────┐
  │  전수 스캔 universal · 다축 프로파일러(5축) · 관계사슬 9개 · 비율 15개          │
  │  변동분해 4브리지 · 커버리지 원장(모집단=분석+제외+미설명) · SCE 2D 검산         │
  └────────────────────────────────────┬───────────────────────────────────────┘
                                        │  materials.py 관점별 발췌(등수 힌트 없음)
                                        ▼
      ┌────────┐┌────────┐┌────────┐┌────────┐   ← 내부 4관점 병렬 발견 (GPT-5.4)
      │ 수치   ││ 주석   ││ 흐름   ││ 추세   │      + industry(동종·GPT) + external(Gemini)
      └────────┘└────────┘└────────┘└────────┘
                    │  grounding: 유효숫자 대조로 환각 탈락 (silent drop 0)
                    ▼
              계정/관계/회사 카드 클러스터 (표수 N/4 · 연속 우선순위 점수)
                    │  변동분해 부착 → 조사원(도구 루프) → 반박 + 외부검증(병렬)
                    ▼
   ┌──────────────────────────────────────────────────────────────────┐
   │  의심건 카드 목록 (Streamlit)                                        │
   │  ① 무엇이 의심스러운가(주장·근거표)  ② 왜 움직였나(분해표·차트)       │
   │  ③ 시각자료   ④ 반박(반대근거·정상설명·확인질문·다음절차)            │
   └──────────────────────────────────────────────────────────────────┘
                    │
                    ▼   L5 Human — 감사인이 승인 / 기각 / 추가질문
```
...
| 4   | `4_SIGNAL-ENGINE.md`          | L2 결정론 신호엔진 — 전수 스캔·5축·관계사슬·변동분해·커버리지 원장 |
| 5   | `5_PERSPECTIVES-CARDS.md`     | L3/L4 6관점 발견 → grounding → 카드 → 조사 → 반박·외부검증         |
| 6   | `6_GROUNDING-GUARDRAILS.md`   | 환각 방지 4중 장치 — grounding·어휘게이트·figure_sheet·금액환산    |
...
| 지표                                   | 값                                                         | 상세                               |
| -------------------------------------- | ---------------------------------------------------------- | ---------------------------------- |
| 표준 계정 매핑                         | 약 **2,015개** canonical (BS/CIS/CF/SCE)                   | [4장](4_SIGNAL-ENGINE.md)          |
