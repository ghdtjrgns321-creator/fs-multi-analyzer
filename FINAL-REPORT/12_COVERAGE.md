...
> **정독 이후 증분**(2026-07-18): 게이트 검문 모듈 3(`gate_identities`·`gate_quality`·`gate_yoy`)과 대응 테스트 3이 추가돼 핵심 시스템은 **312 파일**이 됐다. 신규 6은 3장(게이트)·9장(테스트)에 반영돼 있고, 여기 분모는 최초 전수 정독 시점 기준임을 명시한다 — 사후 증분을 원래 정독 범위처럼 적지 않는다.
