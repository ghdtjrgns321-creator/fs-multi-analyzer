...
**L1 정규화** — `finstate_all_CFS.csv`의 XBRL 행이 canonical로 매핑된다. `재고자산`은 `ifrs-full_Inventories` id로 `EXACT` 매핑. 정규화 DB에 `CFS:재고자산` 시계열이 저장된다: 2016~2020년 **46억 → … → 168,532,992,835원(1,685.3억)** — 4배 단조 증가.

**온보딩 게이트** — G1 완결성 OK, BS 항등식 잔차 100만원 이내, 통화 KRW → `gate_passed=True`. 아스트는 L2로 진입한다.

**L2 신호엔진** — `universal.scan_universal_signals`가 `CFS:재고자산`의 YoY를 스캔한다. 4배 증가는 임계(yoy_pct_abs=50)를 크게 초과해 신호가 뜬다. 동시에 `profiler`의 trend 축(단조성 1.0 × 누적변화/자산)이 높은 분위를 받고, `metrics_panel`이 재고회전율(DIO) 추세(0.84→0.3)를 계산해 전 계정 패널에 부착한다. 관계사슬 `[재고, 매출원가, 재고평가손실]`도 활성화 — 매출원가가 2020년 111억→62억으로 역행했다.
