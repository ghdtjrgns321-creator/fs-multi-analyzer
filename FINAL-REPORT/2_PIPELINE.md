...
flowchart TD
    ...
    Q --> G
    G -->|PASS| OB[LLM 전처리<br/>별칭 자동 등록 ≥0.7 + 5개년 재정규화<br/>본문 읽기→서술형 감사관심 적재]
...
| LLM 전처리         | 게이트 통과 DB  | 별칭 제안·고신뢰(≥0.7) 자동 등록+5개년 재정규화 · 본문 읽기 서술추출                       | quirk·report_extracts·완료 마커     | [3장](3_COLLECT-NORMALIZE.md)    |
...
**종착** — 카드 24장(계정 13·관계 4·회사 7)이 생성되고 그중 **재고자산이 rank 1**로 뜬다. 골든 검사2 채점: recall@5 = 2/3(재고자산 rank1·매출원가는 관계 카드로 rank1·자기자본 미적중). 동시에 골든 검사1은 이 카드에 찍힌 수치 N=59건이 전부 as-filed DART 원값과 일치함을 전자동 확인했다(불일치 0). 2026-08-01 정체 마스킹 실행 기준이다.
