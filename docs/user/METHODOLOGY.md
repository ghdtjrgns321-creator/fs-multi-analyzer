...
> 감사기준 근거 목록은 [../agent/AUDIT_BASIS.md](../agent/AUDIT_BASIS.md)(작성 예정).
...
| 분류 | 실무 지표 (예) | 엮는 계정 조합 |
|------|----------------|----------------|
| 수익성 | ROE, ROA, ROI, 매출총이익률, 영업이익률 | 순이익↔자본/자산, 매출총이익↔매출 |
| 활동성 | 매출채권회전율·DSO, 재고회전율·DIO, 총자산회전율 | 매출↔매출채권, 매출원가↔재고 |
| 안정성 | 부채비율, 유동비율, 이자보상배율 | 부채↔자본, 유동자산↔유동부채, 영업이익↔이자비용 |
| 이익의 질 | 영업CF / 순이익, 발생액 비율 | 영업현금흐름↔당기순이익 |

- 반드시 **출처 있는 자료 기반**으로(LLM 기억만으로 단정 금지). 가능하면 1차 기준과도 연결한다.
- 결과: 기준서엔 없지만 **실무가 중시하는 관계까지 커버. 완전성.**
...
- **1차 (감사기준 전수 → 선별 → 매핑)**: 진행 중.
- **2차 (웹+LLM 실무 지표·계정 조합 발굴)**: 예정.
