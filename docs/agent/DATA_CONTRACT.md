...
> 생성일: 2026-06-01. 2025 데이터 갱신: 2026-06-02.
...
`finstate_all` 응답은 `pandas.DataFrame`으로 수신했다. 실제 컬럼은 2022~2025 CFS/OFS 모두
동일했다.
...
| 2025 | CFS | 229 | 124 | 52 | 17 | 13 | 42 | 105 |
| 2025 | OFS | 141 | 98  | 44 | 14 | 7  | 31 | 45 |

4개년 CFS/OFS 전체에서 관찰된 고유 계정 라벨은 173개다.

---

## 4. 계정 라벨 실제 예시

아래는 4개년 CFS/OFS에서 처음 관찰된 순서 기준 예시다. 정규화 전에는 이 값을 코드에
하드코딩하지 않는다.
...
| 2025 | CFS | 5  | 5  | 단기차입금 등 |
| 2025 | OFS | 2  | 2  | 단기차입금 등 |
...
| 2025 | `20260310002820` | 7 | `entity00126380_2025-12-31.*` 파일 세트 |
