...
`IssueType`은 **재무제표 영역 축**의 통제된 enum이다(분식 가설 축 아님 — 도구 목적은 '이상변화 큐').
`revenue_receivables`, `cost_inventory`, `asset_valuation`, `liability_liquidity`, `equity_capital`,
`contingency_related_party`, `earnings_tax`, `cash_flow`, `unmapped_material_account`, `기타`.
구체 위험(계속기업·이익의 질 등)은 enum이 아니라 subtype 자유서술로 보존한다.

매트릭스(계정 × 검증관점)는 에이전트 편성이 아니라 **리포트 출력 구조**로 존재한다.
행은 신호엔진이 회사마다 동적 선정한 계정, 열은 검증관점이다.

### 카드 후속 검증 단계 — 발견자와 검증자의 분리 (2026-07 재편)

관점은 두 부류로 나뉜다. **발견자**(내부 4관점: 재무 수치·주석·계정 간 관계·시계열 추세)는 카드가 생기기 전
병렬로 넓게 스캔하고, **검증자**는 카드가 만들어진 **뒤** 카드별로 붙어 깊게 판다:

```
내부 4관점 병렬(발견) → grounding → 클러스터=카드
  → 변동 분해(코드, §6.5): "무엇이 움직여서 이렇게 됐나" 표 부착
  → 카드별 후속 검증(병렬):
      · 반박 에이전트 — 카드 주장+분해를 받아 반대근거·정상설명 (기존 S6)
...
- external을 발견자에서 검증자로 옮기는 근거: 카드 없이 막연히 회사명 검색하는 것보다
  분해로 좁혀진 가설("판관비 기여 -54%")을 들고 검색하는 쪽이 질·비용 모두 우위.
- industry(동종)도 같은 방식의 타깃 비교로 옮길 수 있으나 한 번에 하나(후속).
- 표수(N/4)는 내부 발견자만 카운트(기존 유지). 검증자는 근거 배지·본문을 채운다.

### 조사 단계 — 카드별 조사원(도구 루프) (2026-07-10 설계, 2026-07-11 1단계 구현 완료 — 실 LLM 프로브 실측은 대기)

진단(2026-07-10): 파이프라인이 발견(넓게)은 갖췄으나 **조사(깊게)와 판단(결론 주체)이 없다.**
① 각 단계가 릴레이 1회씩이라 앞 답이 다음 질문을 만드는 꼬리무는 조사가 없음 ② 카드 조각마다
주인이 있는데 종합 결론의 주인이 없음 ③ risk_level이 LLM 감 라벨(원칙 §3.1 위반 유일 지점)이고
외부검색 대상 선정까지 전파 ④ 부모-자식 카드(GP·OP)가 서로 모른 채 분리 ⑤ 임계·등급 컷은
"분식은 기준선 아래 숨는다"와 충돌.

해법 구조(1단계 범위 4항목):

```
1) 브리지 병합: decomposition.yaml 부모-자식 계정 카드는 한 카드로
   (자식은 "원인 경로"로 부모 카드에 편입)
2) 조사원(investigator) 신설 — 카드별 후속 검증의 첫 단계:
   결정론 게이트(분해 미설명 잔차율·leaf 기여 집중도)로
     "코드가 이미 원인 설명 완료" → 종합 1호출(결론문만)
     "미해결(잔차 큼·기여 분산)"   → 도구 루프(캡 5~8회):
        decompose_change 재귀·시계열 조회·주석 검색·비율 계산(§8 tool DSL 소비처)
   산출 = 구조화 결론: 원인 경로 · 어디까지 좁혀졌나 · 이상 지점 · 남은 확인사항
3) 결론을 카드 최상단에. 반박·외부검증(기존 검증자)은 결론을 입력으로 받음
4) 연속 점수: risk_level(High/Medium/Low) 폐지 — 코드가 규모(자산 대비)·표수·
   grounding 강도·분해 미설명 잔차·반박 판정으로 연속 점수 산정. 컷 없이 정렬만.
   LLM이 매긴 위험도는 입력에서 제외.
```

- **에이전트 토론 미채택 근거**: LLM 둘을 마주 앉혀도 새 사실은 안 생긴다(같은 입력의 재조합).
  조사가 깊어지는 유입구는 도구 호출뿐 — 왕복은 도구 쪽에 쓴다(원칙 §3.1·§3.4).
- **게이트는 임계 컷이 아니다**: 모든 카드가 결론을 받는다. 코드가 이미 답을 낸 카드만
  LLM 조사를 반복하지 않는 것(경로 차이지 배제가 아님) — ⑤ 문제와 충돌하지 않음.
- **비용**: 조사원 입력은 카드 스코프(3~8k 토큰, 도구 결과 2~5k/회)라 발견 관점(전 데이터
  250k+)보다 훨씬 작음. 삼성 추정 +₩800~2,500(게이트 통과율에 따라). 전면 구현 전
  **카드 2~3장 프로브로 품질·비용 실측** 선행.
- **백로그(2단계 이후)**: 반박을 "같은 도구를 쥔 반대 목표 조사원"으로 승격(조사원 확증편향
  견제, 정상설명 칸의 실질화) / 카드 횡단 스토리 종합(매출 이탈+재고 급증+회전 둔화 = 한 사건).
...
### 6.5 변동 분해 엔진 (variance bridge) — 카드의 "왜"를 코드가 계산

카드가 "영업이익 -62.8% 급감"까지 찾아도 **왜**(매출·원가·판관비 중 무엇의 기여)가 없으면
사람이 검증할 수 없고, 반박·외부 에이전트도 숫자 없이 추측만 한다. 분해는 계산이므로
LLM이 아니라 코드가 한다(원칙 §3.1).

- **항등식은 config** (`config/decomposition.yaml`): K-IFRS 표준 소계 항등식만 둔다 —
  유한·닫힌 집합(GP=매출−원가, OP=GP−판관비[±기타수익·비용])이며 계정 트리 전수 정의가 아니다.
  2026-07 코퍼스 전수 실측(4,782 corp-year): GP 잔차≤1% 4,146/4,147(99.98%),
  OP 표준형 3,787/4,112(92.1%), 기타포함형은 9.9%만 적합 → **회사별로 변형 중 |잔차| 최소를
  데이터로 선택**한다(손 정의 없음).
- **잔차 정직 표시**: 구성 합이 부모 변동과 다르면 "미설명 잔차 X원(Y%)" 행으로 노출한다
  (커버리지 원장과 동일 원칙 — 조용히 맞는 척 금지).
- **세부 2단계 재료**: 주석 detail의 판관비 내역은 실측 3.7%(171/4,614)로 재료 부족 확정
  (XBRL 태그 미강제) → 세부는 IS 개별 비용 계정 + 동행 계정(같은 표 동년 급변 top-N,
  정의 불필요)으로 보강한다.
- **소비처**: ① 카드 "왜 움직였나" 표(L5) ② 반박 에이전트 입력 ③ 외부 검증 에이전트(§5
  후속 검증 단계)의 타깃 검색 쿼리 재료.
...
**(1) 결정론 점검 — 이관 원장 + 항등식 검산** — 두 축으로 묻는다. ①**원본에 있던 게 다 왔나**
(`src/normalize/transfer_ledger.py`): 원본 항목 전량(재무제표 raw 행·SCE 행·주석 fact)을
모집단으로 고정하고 *적재 + 사유 있는 제외 + 미설명*으로 분해, **미설명 0이어야 통과**하며
원본이 없어 대조 못 하면 그것도 통과가 아니다. 임계로 자르지 않는다(비율이 아니라 차집합).
②**온 숫자가 서로 맞나**: BS 항등식 잔차 tol(100만원) 이내 + G3 산술검산 경성 위반 0 +
G7 소계·표간대사 위반 0. 검산 0건은 '통과'가 아니라 '검산 못함'이다.
부수로 G5 신호 dangling 0 · 통화 KRW · G8 차단사유 0(핵심 표 결손·총계 부호)을 본다.
(G2 충돌 인벤토리·G6 dump·G9 연도간 대사는 보고용.)
