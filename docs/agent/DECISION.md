...
## D13. 외부 관점 검색 → agentic query generation + grounded evaluation

- **결정**: 외부 관점은 고정 템플릿 쿼리 대신 3단계로 수행한다.
  1. 내부 review queue, 지표 요약, 최신 신호 스냅샷을 보고 `gemini-2.5-flash`가 검색어
     1~3개를 생성한다.
  2. 생성 검색어별 Google Search grounding으로 `ContextBrief`를 수집하고, 출처 URL이
     grounding chunk와 매칭되는 항목만 남긴다.
  3. 별도 LLM 평가자는 검색 결과와 출처만 보고 외부 관점 `PerspectiveAssessment`를 만든다.
- **이유**: 기존 `회사명 + 연도 + 상위 subject 나열` 쿼리는 지표 약어와 여러 계정이 섞여
  검색 의도가 흐려졌다. 내부 분석을 query rewriter가 검색어로 압축하면 agentic RAG의
  dynamic query rewriter 패턴처럼 검색 품질을 높일 수 있다.
- **경계**: 쿼리 생성은 내부 데이터를 참고할 수 있지만, 외부 관점 평가는 검색 결과와 출처만
  입력받는다. 검색어는 내부 데이터에 나온 계정·지표·증감 사실 기반이어야 하며, `분식`,
  `부정`, `fraud` 같은 추측성 키워드는 제거한다.
- **비오염 원칙**: 외부 관점은 여전히 내부 판단 필드를 바꾸지 않는다. 외부 맥락이 내부 위험을
  설명할 수는 있지만, 내부 위험을 면죄하거나 낮추는 근거로 쓰지 않는다.

## D14. 외부 관점 모델 → Gemini 3.1 Pro preview로 분리

- **결정**: 수치·주석·흐름·변동 4관점은 `gemini_model == "gemini-2.5-flash"`를 유지하고,
  외부 관점의 쿼리 생성과 검색 결과 평가는 `gemini_external_model == "gemini-3.1-pro-preview"`를
  사용한다. 사용 가능한 모델 목록에서 `models/gemini-3.1-pro-preview`를 확인했고, API 호출
  model id는 `gemini-3.1-pro-preview`로 사용한다.
- **이유**: 외부 관점은 내부 이상 신호의 가능한 원인 사건을 찾고 검색 결과를 맥락화해야 한다.
  단순 구조화 요약보다 원인 탐색형 추론 품질이 중요하므로 Flash 계열과 분리한다.
- **경계**: 외부 Pro는 계정별 검색어 매핑 테이블을 받지 않는다. 입력은 회사명, DART 회사개황
  기반 사업 도메인 메타데이터, target year, 내부 상위 신호와 지표뿐이다. 어떤 계정이 어떤
...
- **결정**: PLAN §16.1의 B 접근을 구현해 `industry`를 L4의 6번째 독립 관점으로 추가한다.
  피어는 대상 회사 DART 회사개황의 `induty_code`를 조회한 뒤 해당 industry config 피어만 사용하고,
...
- **비오염 원칙**: 동종업계 관점은 외부 관점(D8/D11)처럼 `risk_level`, `issue_type`,
  ...
  신호로만 쓰며, 대상 회사의 사업구조가 피어와 달라 단순 비교에 한계가 있음을 항상 명시한다.
