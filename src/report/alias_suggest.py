...
→ 회사 단위로: 계산(후보 검색)은 코드, 분류 선택은 LLM, **적용은 사람 확인**(자동적용 금지).

포지셔닝: 부정 확정 아님. 제안은 후보일 뿐, 사람이 회계적으로 확인해 quirk(alias_additions)에
등록한다. LLM은 제시된 candidate 목록 안에서만 고른다(없으면 '기타 중요 계정'으로 보류).
"""
