...
[입력] 회사명 및 사업연도
  ...
  │    계정 정규화     기업별 계정명을 <a href="#표준-계정-사전-구축">표준 사전</a>으로 매핑, 신뢰도 기록   ◀──┐
  │    주석 인덱싱     <a href="#3-3-기술-설명--주석-인덱싱">XBRL 주석 개념 전개, 주제 28종 분류</a>          │
  │                                                                     │
  ├─ 검문 ─────────────────────────────────────────────────────────│
  │    정합성 검문     <a href="#3-4-기술-설명--정합성-검문">이관 원장 + 항등식 검산</a> → 통과 여부 판정        │
  │    데이터 보정     <a href="#3-5-기술-설명--데이터-보정">계정명 연결, 본문 서술 추출</a> ────────────────┘
  │                   등록 시 5개년 재정규화 후 재검문
  │
  ├─ 분석 ──────────────────────────────────────────────────────────
  │    신호엔진        5개년 전 계정 전수 연산 — <a href="#계정별-지표">계정별 지표</a>, <a href="#관계사슬">11개 관계사슬</a>, <a href="#재무비율">15개 재무비율</a>, <a href="#변동분해">변동분해</a>
  │    멀티 에이전트   5관점 독립 실행 — 재무 수치, 주석, 계정 간 관계, 시계열 추세, 업종 비교
  │
  └─ 산출 ──────────────────────────────────────────────────────────
       리포트 병합    인용 수치와 원장 수치 대조 검증, 통과분만 검토 카드로 확정
                      외부 근거 검증은 카드 확정 이후 수행
  ▼
  검토 카드
</pre>
...
2. [실증 예시](#2-실증-예시)
3. 기술 설명
   - 준비 — [데이터 수집](#3-1-기술-설명--데이터-수집), [계정 정규화](#3-2-기술-설명--계정-정규화), [주석 인덱싱](#3-3-기술-설명--주석-인덱싱)
   - 검문 — [정합성 검문](#3-4-기술-설명--정합성-검문), [데이터 보정](#3-5-기술-설명--데이터-보정)
   - 분석 — [신호엔진](#3-6-기술-설명--신호엔진), [멀티 에이전트](#3-7-기술-설명--멀티-에이전트)
   - 산출 — [리포트 병합](#3-8-기술-설명--리포트-병합)
4. [검증](#4-검증)
...
정합성 검문이 지적한 계정을 표준 계정에 잇고, 숫자로 공시되지 않는 위험을 사업보고서 본문에서 추출 — 이 단계만 LLM이 관여

### 미매핑 계정 보정

계정 정규화 단계에서 매핑 되지 않은 계정을 LLM 으로 보정
...
- 정합성 검문이 막혀도 이 단계는 수행 — 한 단계 실패가 전체를 막지 않음
- 재판정 : 여기서 이름을 이어 붙이면 미매핑 비중이 줄어, 검증을 다시 돌렸을 때 통과 가능
