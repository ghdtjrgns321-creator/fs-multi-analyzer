...
> **실제 구현**
> - 정합성 검증 : [`onboarding_gate.py`](src/normalize/onboarding_gate.py), [`gate_identities.py`](src/normalize/gate_identities.py)
> - 별칭 보정 : [`alias_suggest.py`](src/report/alias_suggest.py)
> - 본문 읽기 : [`reader.py`](src/report/reader.py), [`layer1.py`](src/report/layer1.py)

```
전처리 실행 프로세스
  ① 정합성 조건 검증       6개 무결성 조건 검문 → 통과 판정
  ② 보정 및 텍스트 추출    미매핑 계정 별칭 자동 등록, 5개년 재정규화       
                         사업보고서 본문 서술 추출
```

### 3-3-1. 정합성 조건 검증

- 판정 주체 : 전부 코드, LLM은 통과 여부에 관여하지 않음 — ②는 ①이 지적한 것을 고치는 보조 작업
- 차단 이유 : 정규화가 깨진 채 5관점에 들어가면 틀린 수치 위에서 결론 도출 → 카드를 만들어 놓고 걸러내는 대신 진입을 거부하고 원인을 표시, LLM 호출 비용도 선차단
- 미통과 시 : 분석 진입 차단(`can_enter_analysis`), 어느 조건이 왜 막혔는지 단계별 표시 → 별칭 보정이나 회사 특이사항 등록으로 원인을 고친 뒤 재실행
- 예외 : 본문 읽기(LLM) 실패는 차단하지 않음 — 경고 후 사람이 확인하고 강행, 외부 API 장애로 분석이 영구히 막히는 것을 막되 서술형 누락은 숨기지 않음

여섯 조건을 **모두 충족해야** 다음 단계로 진행
...
### 3-3-2. 미매핑 계정 보정, 본문 서술 추출

- 실행 조건 : ①이 떨어져도 계속 수행 — 한 단계 실패가 전체를 막지 않으며, 실제로 막히는 지점은 분석 진입
- 재판정 : 별칭이 등록되면 5개년이 재정규화되므로 ①을 다시 돌리면 미매핑 비중이 줄어 판정이 바뀔 수 있음
