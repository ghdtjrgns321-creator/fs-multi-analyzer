...
3. **Preprocess** — LLM 전처리: 미매핑 계정 별칭 자동 보정(확실한 것만) + 본문 통독으로 서술형 감사관심 적재
...
└────────────────────┬──────────────────────────────────────────────────────┘
                     ▼  관점별 발췌
┌───────────────────────────────── LLM 분석 ─────────────────────────────────┐
│  LLM 전처리(1회)     별칭 자동 보정 · 본문 통독 → 서술 감사관심 적재              │
...
#### LLM 전처리 — 게이트 통과 후, 카드 분석 전

게이트를 통과하면 화면의 [LLM 전처리] 단계가 남은 준비를 맡는다. 버튼 한 번에 자동으로 돌고, 완료 기록이 남으면 재실행을 요구하지 않는다 — 이 기록이 카드 분석(검증 실행) 진입 조건이다. 하는 일은 셋이다.
...
| 3   | 서술형 감사관심(소송·특수관계) | LLM 전처리가 추출 — 실패·0건을 강행하면 누락(경고로 표면화)             |
