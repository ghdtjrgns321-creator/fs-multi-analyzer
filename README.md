...
방향이 중요하다. FAIL은 "이 회사가 수상하다"가 아니라 **"우리가 이 회사를 제대로 못 읽었다"**이다. 그래서 FAIL이면 회사를 버리는 게 아니라 별칭을 보정해 사전을 고치고 다시 검문한다. 이름과 달리 이 게이트는 별도 버튼이 아니라 **[분석 준비]가 수집·정규화 직후 자동 실행**하며, 통과 여부가 "준비완료" 판정을 만든다.
