...
```mermaid
%%{init: {"flowchart": {"curve": "linear", "nodeSpacing": 28, "rankSpacing": 34}}}%%
flowchart TD
    IN(["회사명 · 사업연도"]) --> A
...
| 검문 | [정합성 검문](#3-2-1-정합성-검문)       | 준비 중 유실된 항목 점검 + 대차·합계 검산 → 통과 여부 판정                                                                       |
...
3. 기술 설명
   ...
   - [3-2. 검문](#3-2-기술-설명--검문) — [정합성 검문](#3-2-1-정합성-검문), [데이터 보정](#3-2-2-데이터-보정)
