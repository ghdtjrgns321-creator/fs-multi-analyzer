...
> 상장사 공시 재무제표를 **5개 관점 AI가 교차검증**해 감사인이 확인할 리스크 후보를 카드로 제시하는 도구
...
- 처리 6단계
  ...
  | 단계        | 하는 일                                      | LLM 개입    |
  | ----------- | -------------------------------------------- | ----------- |
  | ① Collect   | OpenDART 원본 수집(본표·주석·사업보고서)     | 없음        |
  | ② Normalize | 회사마다 다른 계정을 표준 계정으로 통일      | 없음        |
  | ③ Prepare   | 파싱 정합성 검증 → 미매핑 보정 · 본문 통독   | 보정·통독만 |
  | ④ Signal    | 전 계정 전수 스캔 후 계산                    | 없음        |
  | ⑤ Discover  | 5개 관점이 각자 발견 → 카드 확정 후 외부검증 | 발견·검증   |
  | ⑥ Report    | 의심 후보를 카드로 묶어 게시                 | 종합·반박   |
...
<p align="center"><sub>② 연도 선택과 실행 — 보유·준비완료 연도를 보여주고, 전처리와 교차검증을 각각 실행한다</sub></p>
