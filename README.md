...
| 분석 | [계산 엔진](#3-3-1-계산-엔진)                   | 통합 재무 지표 산출 — [계정별 지표](#계정별-지표), [11개 관계사슬](#관계사슬), [15개 재무비율](#재무비율), [변동분해](#변동분해) |
...
| 타겟 계정 적중   | **2/3** — 재고자산(재무 수치, 시계열 추세 2관점), 매출원가(계정 간 관계)                           |
| 미탐지           | 자본총계 — 재고자산, 매출원가 왜곡 → 당기순이익 과대 계상 인과관계 미도출                            |
...
<details>
<summary><b>지표 계산식 11종</b></summary>
...
<details>
<summary><b>신호별 기준선 6종</b></summary>
...
| 관점         | 투입 데이터                                       |
| ------------ | ------------------------------------------------- |
| 재무 수치    | 계정 지표 전량, 재무비율 요약과 시계열, 관계 신호 |
| 주석         | XBRL 주석 전량, 사업보고서 본문 서술              |
| 계정 간 관계 | 계정 지표, 관계 신호, 재무비율, 기타 중요 계정    |
| 시계열 추세  | 계정 지표 전량, 비율 시계열, 관계 신호            |
...
- 진입 조건 : 하위 요안 분해 부재 / 미설명 잔차 20% 초과 / 전년 대비 변동 없음 / 세부 항목 데이터 부재 / 단일 최대 기여 항목 60% 미만 중 하나라도 해당 시
...
- 수치 정합성 대조 : 외부 인용 금액을 정수 단위로 정규화 → 원본 수치 대조
...
- 아스트 자본총계 미포착 : 재고자산, 매출원가 왜곡이 당기순이익 과대계상 인과관계 도출 실패
