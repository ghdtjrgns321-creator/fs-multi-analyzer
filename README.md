...
2. **Normalize** — 회사별 XBRL을 DART 공식 사전으로 정규화 + 온보딩 게이트 검문 ([분석 준비] 한 번에 자동)
3. **Onboard**   — LLM 전처리: 미매핑 계정 별칭 자동 보정(확실한 것만) + 본문 통독으로 서술형 감사관심 적재
4. **Signal**    — 계산전용 코드가 계정 전수 스캔 (기준축 5+관계사슬 9+재무비율 15)
...
└────────────────────┬────────────────────────────────────────────────────────┘
                     ▼  관점별 발췌
┌───────────────────────────────── LLM 분석 ─────────────────────────────────┐
│  온보딩 전처리(1회)  별칭 자동 보정 · 본문 통독 → 서술 감사관심 적재       │
...
└────────────────────────┬───────────────────────────────────────────┘
                         ▼
┌─ 온보딩 검문 — 분석 준비가 자동 실행 ──┐              ┌─ 별칭 3단 분업 ──────────┐
│  빠뜨린 계정 · 자산=부채+자본 · 산수   │── 실패 ─────▶│  코드 후보 → LLM 선택    │
│  신호가 볼 계정 존재 · 통화            │◀── 재검문 ───│  → 확실한 것만 자동 등록 │
└────────────────────────┬───────────────┘              └──────────────────────────┘
                         │ 통과
                         ▼
┌─ 온보딩 전처리 — LLM 1회 · 카드 분석 진입 조건 ────────────────────┐
│  미매핑 계정 별칭 제안 → 확실한 것만 자동 등록 후 5개년 재정규화   │
│  (애매하면 등록 없이 "기타 중요 계정"으로 게시 — 조용한 소실 없음) │
│  사업보고서 본문 파트별 통독 → 서술형 감사관심 적재                │
└────────────────────────┬───────────────────────────────────────────┘
                         ▼
...
━━━ [L1] 정규화 — 회사마다 다른 계정명을 한 이름으로 접는다 · LLM 0회 ━━
  ...
  ◆ 계정 이름 맞추기   XBRL 한 줄 → 정준 계정 2,015종 중 하나
      ...
      2순위  한글 계정명    별칭 사전 1,770개
        ...
        │       게이트 보고서의 충돌 목록으로 남겨 정비 때 사람이 본다
...
━━━ 온보딩 게이트 — 회사가 아니라 우리 번역을 검문한다 ━━━━━━━━━━━━━━━
  ...
  이 검문은 화면의 [분석 준비] 버튼이 수집·정규화 직후 자동 실행한다.
  통과한 연도만 "준비완료"가 되어 다음 단계로 갈 수 있다.
  ...
  ◆ 결과에 따른 분기
        │
        ├── 실패 ──▶ 별칭 보정 ──▶ 다시 검문
        │              코드가 후보를 뽑고 → LLM이 고르고 → 확실한 것만
        │              자동 등록해 그 회사만의 특이사항으로 저장
        │              (애매한 것은 별도 정비 화면에서 사람이 등록)
        │              회사를 버리는 게 아니라 우리 사전을 고친다
        ▼ 통과
━━━ 온보딩 전처리 — 게이트 통과 후, 카드 분석 전 LLM 1회 ━━━━━━━━━━━━━
  ...
  ◆ 본문 통독       사업보고서 본문을 파트별로 읽는다
      ...
      → 추출 0건이나 실패면 숨기지 않고 경고를 띄운 뒤
        사람이 확인하고 강행할 수 있다 (조용한 누락 금지)
...
방향이 중요하다. FAIL은 "이 회사가 수상하다"가 아니라 **"우리가 이 회사를 제대로 못 읽었다"**이다. 그래서 FAIL이면 회사를 버리는 게 아니라 별칭을 보정해 사전을 고치고 다시 검문한다. 이름과 달리 이 게이트는 화면의 온보딩 버튼이 아니라 **[분석 준비]가 수집·정규화 직후 자동 실행**하며, 통과 여부가 "준비완료" 판정을 만든다.
...
#### 온보딩 전처리 — 게이트 통과 후, 카드 분석 전

화면의 "온보딩" 단계는 이 게이트가 아니라 **LLM 전처리**다. 버튼 한 번에 자동으로 돌고, 완료 기록이 남으면 재실행을 요구하지 않는다 — 이 기록이 카드 분석(검증 실행) 진입 조건이다. 하는 일은 셋이다.

1. **별칭 자동 보정** — 미매핑 계정에 코드가 같은 표의 후보를 12개 이내로 좁혀 주면 LLM이 그 안에서만 고르고, 신뢰도 0.7 이상만 자동 등록한 뒤 5개년 전체를 재정규화한다(전년도 미매핑이 올해 "신규 발생"으로 오인되는 것을 막는다). 애매한 제안은 등록하지 않고 "기타 중요 계정"으로 게시한다 — 사람이 직접 등록하는 폼은 별도 정비 화면에만 남아 있다.
2. **본문 통독** — 사업보고서 본문을 파트별로 읽어 소송·특수관계·우발부채 같은 서술형 감사관심을 추출·저장한다. 주석 관점이 이를 입력으로 쓴다. 추출 0건·실패는 숨기지 않고 경고를 띄운 뒤 사람이 확인하고 강행할 수 있다.
3. **게이트 재검문** — 준비 단계에서 통과한 검문을 한 번 더 돌려 최신 상태를 확인한다.

참고로 G6 덤프(LLM이 통째로 읽을 수 있게 뽑은 점검 파일)는 게이트가 계속 생성하지만, 이를 통독하는 LLM은 배선돼 있지 않다 — 온보딩 LLM의 역할은 위 별칭 보정과 본문 통독으로 확정됐고, 감사 소견은 관점 분석이 전담한다.
...
병목은 Phase2 카드(삼성 115.8초)와 사업보고서 본문 처리(삼성 ₩1,089·304,852토큰)다. 단가는 입력 $2.5/출력 $10 per 1M·₩1,380/$ 가정이다(실단가 미확정). LLM 에이전트 10개(온보딩 전처리 3 + 관점 6 + 반박 1)가 호출 수와 일치한다 — **측정 당시 구성**이다(당시 external이 발견 관점에 포함된 6관점, 현행은 5관점 + 카드 확정 후 외부검증).
...
| 3   | 서술형 감사관심(소송·특수관계) | 온보딩 전처리가 추출 — 실패·0건을 강행하면 누락(경고로 표면화)          |
