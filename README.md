...
**2. 연도 선택, 실행** — 분석 사업연도 선택 후 [분석 실행] 하나로 원본 수집 · 표준 계정 변환 · 사업보고서 본문 읽기 · 멀티 에이전트 교차검증을 순서대로 수행 (이미 끝난 단계는 건너뜀)
...
**④ 검토 목록 상세 조회** — 결론, 원인 경로, 관점별 주장, 5개년 수치 시각화, 반대 가능성, 확인 질문, 후속 절차 통합 제공
...
| 특징                | 무엇이 달라지나                                                 |
| ------------------- | --------------------------------------------------------------- |
| 숫자는 코드가 계산  | 증감률, 비율, 기여도를 LLM에 안 맡겨 같은 입력이면 같은 수치    |
| 발견 관점 5개 고정  | 숫자, 주석, 흐름, 추세, 업종이 매번 같은 자리에서 검토          |
| 전 계정 전수 스캔   | 샘플링 없이 5개년 전 계정을 훑고 관점이 지목한 것만 카드로 승격 |
| 인용 수치 사실 대조 | 장부에 없는 금액을 든 지적은 사유를 남기고 탈락                 |
| 카드 형식 고정      | 회사와 연도가 달라도 같은 자리에서 비교                         |

어떤 계정을 볼지는 계산 결과가 정하고, 관점을 계정별로 늘리지 않는다<br>
화면 라벨은 6개 — 외부검증은 발견자가 아니라 카드 확정 후의 검증자다
...
- 방대한 데이터 분량
  - 단일 기업 1개년 기준 : 연결/별도 재무제표 + 주석 + 사업보고서 본문
  - 시계열 분석시, 검토해야 할 데이터 급증
- 공시 정보의 형태가 이질적 
  - 재무제표, 주석, 서술 문서 등 정보의 기준축이 달라 데이터가 파편화됨
  - 수동 대조 필요, 문서 간 교차 분석으로 획득 가능한 분석 누락 가능성
- 비표준화된 경험 의존적 검토
  - 데이터 검토의 우선순위 및 체계적 기준 부재
  - 검토자의 개인 역량이나 상황에 따라 분석 결과의 편차 발생
...
| 온보딩 게이트    | 파싱 정합성 검증 → 미매핑 보정 · 본문 읽기   | 보정·읽기만 |
