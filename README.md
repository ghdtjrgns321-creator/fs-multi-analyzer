...
확정된 분식 사건 하나를 **정정 공시 전 원본**으로 처음부터 끝까지 돌린 기록이다. 답을 아는 상태에서 도구가 그 답을 스스로 짚는지, 어디까지 못 짚는지를 본다.

### 4.1 아스트는 왜 분식으로 판정됐나

아스트는 항공기 부품 제조사다. **이미 판매된 재고자산을 매출원가로 비용 처리하지 않고 여전히 보유 중인 것으로 회계처리**했다. 팔린 물건이 장부에는 창고에 남아 있는 상태가 되므로, 비용이 빠진 만큼 자기자본과 당기순이익이 함께 부풀려진다. 재고자산수불부(입출고 장부)를 조작해 외부감사도 방해했다.

| 항목      | 내용                                                                       |
| --------- | -------------------------------------------------------------------------- |
| 분식 연도 | 2018~2021                                                                  |
| 수법      | 판매된 재고의 원가 미처리 → 재고자산·자기자본·당기순이익 과대              |
| 조치      | 회사 과징금 22.4억 · 전 대표 과징금 10.2억 · 증권발행제한 12월 · 검찰 고발 |
| 감사      | 감사인 지정 3년 (상장관리조치는 경영진 교체 감안해 면제)                   |

즉 **정답 계정은 재고자산·매출원가·자기자본** 세 개다. 이 세 개가 카드 큐 위쪽에 스스로 올라오는지가 이번 검증의 채점 항목이다.

### 4.2 입력 — 정정 전 원본으로 고정

분식 사건은 나중에 재작성된 수정본이 남는다. 수정본을 입력으로 쓰면 도구가 이미 교정된 숫자를 보게 되므로 검증이 성립하지 않는다.
...
━━━ 입력 고정 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  아스트 2020 사업보고서 제출 이력
       ...
       │
       └─ 이후 정정·재작성본  ──▶  제외
                                   교정된 숫자를 보고 맞히면 채점 의미 없음

  로드 연도도 2015~2020으로 제한 — 2021년 이후 재표시 값이 흘러들지 않게
```

연결 기준 재고자산은 원본과 우리 DB 값이 일치함을 따로 대조했다(재작성은 별도재무제표만 바꿨다).

### 4.3 도구가 내놓은 것 — 카드 24장

LLM 관점 5개가 각자 독립으로 본 결과를 병합해 **계정 12 · 관계 5 · 회사 7 = 24장**이 나왔다. 큐 최상단이 재고자산이다.
