...
|      | [조사 에이전트](#심층-조사-에이전트)            | 내부 데이터로 원인이 설명되지 않는 카드를 도구로 자율 탐색                |
|      | [반박 에이전트](#반박-에이전트)                 | 카드 전량 일괄 투입 — 논리 충돌과 상대적 중요도 교차 검증                 |
|      | [외부 검증 에이전트](#외부-검증-에이전트)       | 검색으로 외부 근거 수집, 인용 금액을 공시 수치와 대조                     |
