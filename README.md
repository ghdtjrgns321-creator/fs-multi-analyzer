...
### 4.7 사전지식 오염 점검 — 정체를 가리고 재실행

아스트는 언론에 보도된 확정 사건이라 LLM이 이미 알고 있을 수 있다. 그렇다면 재고자산을 짚은 것이 데이터 분석이 아니라 기억일 수 있다. 관점 프롬프트에 "외부 사실·뉴스·업종 기억 금지"가 있지만 이건 지시일 뿐 강제가 아니다.

그래서 입력에서 회사 정체를 지우고 같은 분석을 다시 돌렸다.
...
회사가 어디인지 모르는 상태에서도 같은 계정이 같은 관점 조합으로 다시 올라왔다. 사전지식으로 찍었다는 근거는 나오지 않았다.

한계도 같이 남긴다.

- 1회 실행 대조다. LLM이 비결정적이라 하위 카드 구성은 3장 달랐다(단기차입금·매각예정자산 → 유동부채·유형자산 관계)
- 가린 것은 발견 단계뿐이다. 조사·반박 단계는 회사명을 볼 수 있다(카드가 확정된 뒤라 선정에는 영향 없음)
- 금액 규모·계정 구성만으로 회사를 추정하는 것까지 막지는 못한다

마스킹 자체도 실측으로 검증했다. 첫 시도에서 구멍 3개(접두어 없는 맨 계열사명·프로필 원본 필드·영문 사명)가 드러나 보완했고, 보완 후 5개 관점 재료 전체에서 정체 표식 0건이다.
...
- 드롭하지 않고 **원문 이름을 유지한 채 "기타 중요 계정"으로 게시** — 금액 소실 0
- 다만 표준 이름이 없으면 관계사슬·재무비율에서 조회되지 않아, 사전 누락분은 따로 줄여야 함
