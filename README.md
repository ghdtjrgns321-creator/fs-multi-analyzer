...
<!-- 비교용 임시 블록 시작 — 하나 고르고 나머지는 삭제 -->

**현행안** — 분석 단계에 `멀티 에이전트` 노드
...
**A안** — `관점 병렬 분석`으로 개명 + 산출에 조사·검증 노출
...
flowchart LR
    ...
    subgraph ANALYSIS["분석"]
        direction TB
        G["계산 엔진"] --> H["관점 병렬 분석<br/>5관점 격리 실행"]
    end

    subgraph OUTPUT["산출"]
        direction TB
        I["검토 카드 생성"] --> K["조사 에이전트"] --> L["반박 · 외부 검증"] --> J["사실 대조"]
    end
...
|      | [멀티 에이전트](#3-3-2-멀티-에이전트)           | 각 LLM 독립 실행 — 재무 수치, 주석, 계정 간 관계, 시계열 추세, 업종 비교  |
| 산출 | [검토 카드 생성](#3-4-1-검토-카드-생성)         | 중복 지적 병합, 조사, 반박과 외부 검증을 거쳐 검토 카드 확정              |
...
- **분석 관점 고정, 동적 데이터 라우팅**
  ...
  - 동적 계정 유입: 계산 엔진 연산 결과에서 검토가 필요한 계정만 멀티 에이전트 단계로 유입
...
3. 기술 설명
   ...
   - [3-3. 분석](#3-3-기술-설명--분석) — [계산 엔진](#3-3-1-계산-엔진), [멀티 에이전트](#3-3-2-멀티-에이전트)
...
- 산출 및 활용 : 리스크 추출 결과를 구조화하여 적재, 이후 에이전트 분석(3-3-2. 멀티 에이전트)의 근거 재료로 투입
...
코드 연산을 통해 멀티 에이전트 판단에 사용될 데이터를 산출
