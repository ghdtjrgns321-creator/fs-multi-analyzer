...
사슬은 **어떤 계정을 함께 볼지 정한 축**이고, 실제로 계정을 맞대는 계산은 증가율 괴리 10종 · 방향 비교 4종 · 방향 조합 2종이다.

축을 넓힐 때 계정 이름이 사전에 있다는 것만으로는 부족하다 — **그 계정이 회사 재무제표에 실제로 적재되는지**를 재야 한다. 표본 200사(2개 연도 모두 보유 기준)로 측정한 결과가 짝을 정했다.

| 후보 짝                          | 신호가 뜰 수 있는 회사 | 채택             |
| -------------------------------- | ---------------------- | ---------------- |
| 현금및현금성자산 ↔ 금융수익      | 171사                  | ○                |
| 단기금융상품 ↔ 금융수익          | 101사                  | ○                |
| *(대조군)* 매출 ↔ 매출채권       | 89사                   | 기존             |
| 장기대여금증가 ↔ 투자활동CF      | 63사                   | ○                |
| 현금·단기금융상품 ↔ **이자수익** | 10사                   | ✕ 커버리지 부족  |
| 무형자산 ↔ **무형자산손상차손**  | 25사(실산출 2사)       | ✕ 규모 임계 미달 |
| 무형자산 ↔ **연구개발비**        | 5사                    | ✕ 커버리지 부족  |

이자수익은 대부분 금융수익에 합쳐져 별도 계정으로 안 나온다. 손상차손은 짝은 있어도 금액이 규모 임계(`max(1억, 자산총계 1%)`) 아래라 괴리 신호로는 구조적으로 안 걸린다 — 매핑으로 고칠 수 있는 문제가 아니라 축 자체를 뺐다. 개발비 자산화는 본문 합계 계정으로 측정이 불가능해 주석 관점이 서술로 본다.
...
> **적용 범위** — 이 도구는 **비금융 일반 상장사(제조·판매·건설)** 대상이다. 관계사슬·재무비율·변동분해가 매출–매출원가–재고 구조를 전제하므로 금융·보험·지주·리츠 등은 다루지 못한다. 검증도 연간 재무제표만(분기 제외)·2015년 이후 데이터로 한다.

### 검증 스코어카드

정답(정해진 답)이 **있는** 축만 수치로 보고하고, 정답이 **없는** 축은 "측정 안 함"으로 남긴다 — 정답이 없는 문제에서 가짜 정밀도를 만들지 않기 위해서다.

| 무엇을 봤나             | 정답 기준             | 결과                                                                          | 뜻                                                 |
| ----------------------- | --------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------- |
| **① 수치 정확성**       | DART 원본값           | 카드에 찍힌 숫자 전량 대사 — LG생활건강 72/72 · 아스트 53/53, 환각·환산오류 0 | 화면에 보이는 숫자에 지어낸 게 없다                |
| **② 분식 계정 적중**    | 증선위·법원 확정 분식 | 아스트 재고자산 1순위·recall@5=2/3 · 셀트리온 2/2                             | 진짜 분식 계정이 큐 상위에 뜬다                    |
| **③ 신호 회귀 기준선**  | 확정 분식 6사         | 5/6 발굴(세토피아만 놓침) · 정상기업 삼성·KAI 안 뜸                           | 성능이 아니라 "리팩터가 신호를 안 깨뜨렸다"는 가드 |
| **④ 큐 정밀도(오탐률)** | **없음**              | **측정 안 함**                                                                | "이상변화 큐"라 오탐 정답 자체가 없다              |

- ② 정답 사건 8사는 전부 비금융이며, 실행 비용 때문에 그중 2사만 실제로 돌렸다(표본 적음, 순위는 실행마다 흔들림).
- ③ 삼성에서 뜬 14건은 "오탐"이 아니라 정당한 이상변화(당기순이익·차입금 등)다 — 애초에 오탐 정답이 없다.

이하는 대표 사례로 **아스트 2020년**의 정정 공시 전 원본을 넣은 END-to-End 실행을 상세히 본다.
...
| 정답 계정 | 카드                          | 어떻게 잡혔나                     |
| --------- | ----------------------------- | --------------------------------- |
| 재고자산  | **생성**                      | 숫자·추세가 각각 독립으로 지적    |
| 매출원가  | **생성** (매출↔매출원가↔재고) | 흐름 관점이 세 계정의 괴리로 지적 |
...
| 4   | 1회 실행 비용                  | 소형사 실측 ₩1,365 · 158초. 대기업·금융사는 더 듦 — 수천 사 전수 운영은 비용 부담  |
| 5   | 재실행 결과 미보장             | 결국 LLM이라 같은 입력에도 카드 수가 달라짐. 점수·클러스터 골격은 결정론이라 불변  |
