...
실제로 준비완료 3개 회사연도에 돌리면 회사당 **18~20건**이 실행되고 위반 0이다. 같은 실행에서 경고는 남는다 — 세 회사 모두 **자본변동표의 71~88%가 미매핑**이었고(금액 비중은 재무상태표 기준이라 0%로 보이던 구간), 한 회사는 사업보고서 서술 추출이 0건이었다. 차단 사유는 아니지만 "그 회사의 자본 흐름은 우리가 못 읽고 있다"는 사실이 이제 화면에 뜬다.
