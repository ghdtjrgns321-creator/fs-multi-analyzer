...
사슬은 **어떤 계정을 함께 볼지 정한 축**이고, 실제로 계정을 맞대는 계산은 그 아래 항목들이다 — 증가율 괴리 12종, 방향 비교 4종, 방향 조합 2종. 뒤 셋(손상·현금실재성·특수관계자)이 이번에 추가된 축이다.

| 새 축                | 무엇이 이상하면 걸리나                                                                                 |
| -------------------- | ------------------------------------------------------------------------------------------------------ |
| 자산 손상·자산화     | 무형자산은 계속 느는데 연구개발비가 줄거나, 손상차손이 안 잡힌다 — 비용으로 나갈 것을 자산에 얹은 모습 |
| 현금·금융자산 실재성 | 현금·단기금융상품 잔액은 있는데 이자수익이 따라 늘지 않는다                                            |
| 특수관계자 자금 유출 | 대여금이 늘어나는 방향과 투자활동현금흐름이 어긋난다                                                   |
...
`config/decomposition.yaml`. 매출총이익 → 영업이익 → 세전이익 → 당기순이익을 재귀로 연결해, 각 소계의 전년비 변동을 구성 항목의 기여로 쪼갠다.
...
브리지는 4개지만 회사마다 계정 구성이 달라 변형이 7개이고, 잔차 절대값이 가장 작은 것을 고른다.

**미설명 잔차를 지우지 않고 남기는 게 핵심이다.** 잔차가 크다는 건 분해가 원인을 설명하지 못했다는 뜻이고, 그 사실 자체가 뒤에서 조사원 도구 루프를 여는 트리거가 된다(§3.8).
...
━━━ 재료 준비 — 등수 힌트를 주지 않는다 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ◆ 넘기기 전 두 가지 처리
      ...
      용어 차단   재료에 들어간 내부 키 이름을 자동 수집해 금지어로 등록
                  → 결론 문장에 새어 나오면 다시 쓰게 만든다
...
━━━ [L3] 발견 — 5개 관점이 동시에, 관점당 LLM 1회 ━━━━━━━━━━━━━━━━━━━
  ...
  ◆ 출력 형식을 강제한다 — 자유 서술 덩어리는 받지 않는다
      지적 대상   ┳ 특정 계정    → 계정 ID와 인용 금액 필수
                  ┣ 계정 간 관계 → 대표 계정 + 관련 계정 1개 이상 필수
                  ┗ 회사 전체    → 외부 사건·소송·지배구조 이슈
      영역 분류   10종 중 하나로 고정 — 분식 수법이 아니라 재무제표 영역
                  (부정을 전제하지 않고 "어느 영역의 우려인가"만 표시)
      설명        비어 있으면 거부 (알맹이 없는 지적 차단)

  ◆ 관점이 못 건드리게 코드가 덮어쓰는 것
      관점 이름   LLM이 스스로 붙인 라벨을 믿지 않고 코드가 다시 박는다
      실행 결과   정상 응답(0건도 정상) / 의도적 건너뜀 / 호출 실패 를 구분
                  └ 실패를 건너뜀으로 묻으면 "위험 후보 없음"으로 둔갑하기 때문
...
━━━ 사실 대조 — 인용한 숫자가 실제로 있나 · 코드가 단독 판정 ━━━━━━━━
  ...
  ◆ 판정   본표·주석·자본변동표의 계정 이름 → 실존 금액 색인을 만들어 대조
      계정 지적   계정이 색인에 없음 ──▶ 탈락 "존재하지 않는 계정"
                  인용 금액이 실제 값에 없음 ──▶ 탈락 "실제 값과 불일치"
                  숫자 인용이 애초에 없음 ──▶ 통과 (추세·비율 주장)
      관계 지적   연결된 계정이 하나라도 허구 ──▶ 탈락 (없는 관계 날조 차단)
...
| 업종 | 동종사 지표 · 업종 규칙 · 비율 시계열 · **우선순위 상위 5건**                |

- **예외 하나** — "등수 힌트 없음"은 5관점 중 4관점에만 참이다. 업종 관점만 우선순위 상위 5건을 받는데, 참고용이라 최종 판단에는 관여하지 않는다.
- **외부검증은 여기 없다** — 발견자가 아니라, 카드가 확정된 뒤 특정 카드를 겨냥해 검증하는 역할로 뒤에 배치했다(§3.8).
