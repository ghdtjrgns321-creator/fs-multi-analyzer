...
━━━ LLM 전처리  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ◆ 별칭 자동 보정 - 표준분류가 안 된 계정 이름을 사전에 등록
      ...
      → 신뢰도 0.7 미만은 자동 등록하지 않고 사람에게 넘긴다
          └ 사람이 보는 화면에서, 제안 이름이 관계사슬·재무비율이 쓰는 계정이면 표시
            (이름이 없는 동안은 그 분석에서 조회조차 안 돼 등록 효과가 가장 큼)
          └ 사람이 등록하지 않으면 "기타 중요 계정"으로 게시
...
- 다만 표준 이름이 없으면 관계사슬·재무비율이 조회하지 못함 — **그 건수·금액을 파생층 원장이 따로 센다**(삼성 5건 · 대주 10건)
- 자본변동표 1,494건 전수 조사로 판별 — 여러 회사 공통이면 사전 누락, 1~2사 고유면 원문 유지
- 별칭을 붙이면 그대로 편입되므로, 전처리 화면이 "등록하면 관계사슬·재무비율에 편입되는 계정"을 표시
...
| 3   | 데이터 범위                     | 분기 제외 · 2015 이전 미수집 · 제조업 한정                                                |
| 8   | 주석 숫자표 신규/소멸 미지원   | 분류 축이 매년 재편돼 오탐 34~64%. 계정·자본변동표·서술형은 지원                          |
| 9   | 미매핑 계정의 관계분석 미진입  | 이름을 못 붙이면 관계사슬·재무비율이 조회 못 함. 소실은 아니고(계정 지표·카드엔 포함) 건수·금액을 원장이 표시하나, 잔량(1~2사 고유 라벨)은 구조적 |
