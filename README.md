...
|      | [조사 에이전트](#심층-조사-에이전트)            | 내부 데이터로 원인이 설명되지 않는 검토카드를 자율 탐색                   |
|      | [반박 에이전트](#반박-에이전트)                 | 검토 카드 전량 일괄 투입 — 논리 충돌과 상대적 중요도 교차 검증            |
...
- **분석 관점 고정, 동적 데이터 라우팅**
  ...
  - 외부 검증 절차: 검토 카드 확정 후 외부 데이터 교차 검증 에이전트 실행 → 검토 카드에 분석 지표로 더해짐
- **원본 대조를 통한 환각 차단**
...
| 누락 대조표           | 분석 대상에 포함된 정상 데이터 ↔ 누락/배제된 데이터 교차 검증 결과 | 화면에 미분석으로 표시                    |
...
| 충당부채, 우발부채 교차  | 충당부채, 장기충당부채, 우발부채주석                       |
...
2. 에이전트 간 상호 참조 격리
  ...
  - 다중 관점 합의 신뢰도 확보 : 격리된 환경에서 도출된 결론의 교집합 → 신뢰도 확보
...
| 조사 | 변동 원인이 내부 데이터로 설명 되지 않을 경우 자율 탐색 에이전트로 심층 조사 수행                 |
| 검증 | 반박 에이전트, 외부 검증 에이전트 실행                                                            |

#### 심층 조사 에이전트

- 진입 조건 : 하위 요인 분해 부재 / 미설명 잔차 20% 초과 / 전년 대비 변동 없음 / 세부 항목 데이터 부재 / 단일 최대 기여 항목 60% 미만 중 하나라도 해당 시
- 자율 탐색 루프 : 에이전트가 관점에 제한되지 않고 자율 탐색 - 최대 8회 호출
- 예외 처리 : 결론 실패 시 조사 미수행 표시

#### 반박 에이전트

- 1차 생성 카드 전량에 일괄 투입 : 전체 카드 간의 상호 논리적 충돌 여부 및 상대적 중요도 교차 검증
- 산출물 조작 불가능 : 임의 카드 삭제 및 신규 생성 불가
...
3. 표마다 구조가 다름
   ...
   - 재무상태표 자본 항목과 상호 검증 — 구성요소별 기말 잔액 합계를 재무상태표 자본총계와 검증
4. 회사, 연도별 표기 방식이 바뀜
