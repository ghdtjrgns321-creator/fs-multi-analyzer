...
```mermaid
flowchart LR
    IN(["회사명 · 연도"]) --> L0["L0 수집"] --> L1["L1 정규화"] --> L15["L1.5 주석 인덱서"]
    L15 --> G["온보딩 게이트"] --> L2["L2 신호엔진"] --> L3["L3 역할 에이전트"] --> L4["L4 리포트"] --> OUT(["검토 카드"])
```

처리 7단계 — 단계 이름은 설계 문서의 계층(Layer, 줄여서 L)과 같다

| 단계                         | 하는 일                                                                                                                   | LLM 개입   |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------- |
| Layer 0 (L0) 수집            | OpenDART에서 5개년 재무제표(JSON), 주석(XBRL), 사업보고서 원문을 회사와 연도별로 내려받아 저장                            | 없음       |
| Layer 1 (L1) 정규화          | 회사마다 다르게 쓴 계정명을 표준 계정 사전으로 통일, 계정 ID를 우선 대조하고 매핑 신뢰도를 함께 기록                      | 없음       |
| Layer 1.5 (L1.5) 주석 인덱서 | 주석 원문을 섹션으로 쪼개 해당 계정에 연결, 전기와 당기를 정렬해 문구 변화를 뽑아냄                                       | 없음       |
| 온보딩 게이트                | 이관 완전성부터 통화 일치까지 6개 조건으로 검문, 미매핑 계정은 보정 후 재정규화, 사업보고서 본문에서 서술형 감사관심 추출 | 보정, 읽기 |
| Layer 2 (L2) 신호엔진        | 5개년 전 계정 전수 스캔, 증감률, 변동폭, 추세, 업종 분위, 관계사슬 11종, 재무비율 15종                                    | 없음       |
| Layer 3 (L3) 역할 에이전트   | 5개 관점(재무 수치, 주석, 관계, 추세, 업종)이 각자 의심 항목 선별, 카드 확정 후 외부검증                                  | 발견, 검증 |
| Layer 4 (L4) 리포트          | 인용 금액을 장부와 대조해 근거 없는 지적은 탈락, 남은 후보를 같은 대상끼리 병합해 카드로 게시                             | 종합, 반박 |
