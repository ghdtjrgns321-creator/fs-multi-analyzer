...
처리 7단계 — 단계 이름은 설계 문서의 계층(Layer, 줄여서 L)과 같다
  ...
  회사명 · 연도
      │
      ├─ L0  수집            OpenDART에서 5개년 재무제표·주석·사업보고서 원문을 회사/연도별로 저장
      │
      ├─ L1  정규화          회사마다 다르게 쓴 계정명을 표준 계정 사전으로 통일, 매핑 신뢰도 기록
      │
      ├─ L1.5 주석 인덱서    주석을 섹션으로 쪼개 계정에 연결, 전기·당기를 정렬해 문구 변화 추출
      │
      ├─ 온보딩 게이트       6개 조건으로 이관 검문, 미매핑 계정 보정 후 재정규화, 본문 서술 추출
      │
      ├─ L2  신호엔진        5개년 전 계정 전수 스캔 — 증감·추세·업종 분위·관계사슬 11·재무비율 15
      │
      ├─ L3  역할 에이전트   5개 관점이 각자 의심 항목 선별, 카드 확정 후 외부검증
      │
      └─ L4  리포트          인용 금액을 장부와 대조해 근거 없는 지적 탈락, 남은 후보를 카드로 병합
      │
      ▼
  검토 카드
```
