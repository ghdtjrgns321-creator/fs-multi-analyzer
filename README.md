...
- 첫 실측에서 "자본변동표의 71~88%가 미매핑"이라는 경고가 떴는데, 파보니 **경고 자체가 버그**였다 — 분석이 실제로 쓰는 테이블이 아니라 안 쓰는 표를 재고 있었다(측정 대상을 바로잡으면 0~35%). 이 추적이 더 큰 결함을 끌어냈다: 미매핑 자본거래는 이름이 "기타 중요 계정"이라는 한 덩어리로 뭉개져, 자기주식 매입 582억이 정체 없이 흐르고 있었다. 이름 복원 + 전 수집 데이터 실측 기반 별칭 확충으로 표준분류 밖 자본거래를 32~35% → **10~14%**로 줄였다(잔존은 "기초 보고금액" 같은 구조 라벨).
