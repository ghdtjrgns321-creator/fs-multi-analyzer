...
아스트 2020년 — 실제로 재고자산 과대계상이 확정된 회사의 **정정 전 원본 재무제표**를 입력으로 넣은 화면이다.

> 회사를 고르면 보유·준비완료 연도가 보이고, LLM 전처리 완료 후 저장된 검토 큐가 열린다

![검토 큐 — 아스트 2020 계정별 의심 후보](docs/images/ui-cards-01.png)

> 재고자산 카드 — 결론·관점 주장·추이 차트·외부 근거(제재 보도 링크, 공시와 다른 수치는 ⚠ 마킹)·반박 4요소가 한 장에 담긴다

![의심건 카드 — 아스트 2020 재고자산](docs/images/ui-cards-02.png)

> 관계 카드 — 매출·매출원가·재고자산을 첫해=100으로 지수화해, 매출이 꺾이는데 재고만 오르는 괴리가 선의 벌어짐으로 보인다

![관계 카드 — 매출 ↔ 매출원가 ↔ 재고자산](docs/images/ui-cards-03.png)
...
단 자본변동표는 반대다. 회사가 표준 ID 슬롯에 다른 변동을 신고하는 일이 흔해 **한글 라벨이 실질**이라, SCE 변동행만 라벨을 우선한다.

#### 표준분류에 없는 계정 — 버리지 않되, 이름을 뺏지도 않는다

사전에 없는 계정은 "기타 중요 계정"이라는 바구니로 강등해 보존한다. 문제는 이 바구니 이름이 **계정을 식별하는 칸 자체를 덮어쓴다**는 것이었다. 본표는 원문 라벨을 따로 실어 무사했지만 자본변동표는 그러지 않아, 자기주식 매입·주식선택권 행사·신종자본증권 재분류가 전부 같은 이름으로 합쳐져 흘렀다. 무슨 거래인지 모르니 LLM이 지적할 수 없고, 신규·소멸 판정도 한 덩어리로 뒤섞였다.

지금은 **미매핑이면 회사가 쓴 원문 이름을 정체성으로 되돌린다.** 상태는 상태 칸(`mapping_status`)이 갖고, 이름 칸은 이름만 갖는다.

사전 자체도 넓혔다. 감으로 별칭을 넣지 않고 **수집한 전 회사 데이터에서 미매핑 라벨을 집계**해(자본변동표 보유 1,494 회사연도 · 미매핑 라벨 1,198종), 뜻이 명확하고 여러 회사에 걸친 것만 승격했다. 데모 3사 기준 표준분류 밖 자본거래는 32~35% → **0~5%**가 됐고, 1~2개사에만 있는 꼬리 라벨은 억지로 매핑하지 않고 원문 이름으로 흘린다.
...
━━━ 정합성 검문 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  데이터 수집 경로에서 데이터를 망가뜨린게 없나 검사

  ◆ 통과 조건   6개 전부 만족
      ① 옮기다 빠뜨린 것 없나   계정 소실 여부 · 자산 = 부채 + 자본이 맞나
                                (오차 100만원까지 허용) · 자본변동표 표준화 성공 여부
      ② 표 안의 산수가 맞나     합계 = 구성요소인가를 표마다
                                자산총계 = 유동 + 비유동 · 부채총계 = 유동 + 비유동
                                자본총계 = 지배지분 + 비지배지분
                                당기순이익 = 지배귀속 + 비지배 = 계속영업 + 중단영업
                                총포괄손익 = 당기순이익 + 기타포괄손익
                                매출총이익 = 매출 − 매출원가 · 세전이익 ↔ 법인세 크기
                                └ 구성요소가 아예 없으면 실패가 아니라 건너뜀
                                  (은행은 유동/비유동 구분이, 지주는 매출원가가 없다)
      ③ 두 표에 적힌 같은 값이 같나
                                재무상태표 현금 = 현금흐름표 기말현금
                                재무상태표 자본총계·지배지분·이익잉여금
                                  = 자본변동표의 기말 잔액
                                └ 표 하나만 잘못 붙어도 드러난다. 업종·부호 규약과
                                  무관해서 정상 회사가 걸릴 일이 없다
      ④ 우리가 이 회사를 얼마나 읽었나
                                미매핑 금액이 자산의 20%를 넘으면 차단
                                재무상태표·손익계산서가 없으면 차단
                                총계 계정이 음수면 부호를 잘못 옮긴 것 → 차단
      ⑤ 신호엔진이 볼 계정이 실제로 있나
  ...
  ◆ 통과는 시키되 화면에 띄우는 것   품질은 떨어지지만 분석은 성립하는 상태
      표별 미매핑 비율(절반 넘으면 그 표는 못 읽은 것) · ID와 한글명이 어긋난 계정 수
      주석·사업보고서 본문이 비었나 · 영업이익 = 매출총이익 − 판관비 불일치
      연도 간 대사 — 올해 보고서의 "전기" 칸과 작년 보고서의 "당기"를 맞대
        어긋난 계정을 표기변경/재표시 후보로 구분해 띄운다
        └ 재표시는 회사의 사실(정정공시)일 수 있어 차단하지 않고 검토 재료로 넘긴다
```
...
| **G7** 소계·대사    | 표 안 소계 항등식 9종 + 표 간 대사 4종 (`gate_identities.py`)                                      | 총계만 맞고 내부가 깨진 매핑 · 한 표에만 생긴 오배정                        |
| **G8** 번역 품질    | 미매핑 금액 비중 · 표 커버리지 · 총계 부호 · 원천 적재 (`gate_quality.py`)                         | 절반만 읽은 회사를 "분석 완료"로 내보내는 것                                |
| **G9** 연도 간 대사 | 올해 보고서의 전기 칸 = 작년 보고서의 당기인가 (`gate_yoy.py`, 표면화 전용)                        | 재표시·표기변경·정규화 드리프트가 조용히 흐르는 것                          |

G2(id-라벨 충돌 인벤토리)·G6(LLM 통독 dump)은 **보고용**이라 통과 판정에 안 들어간다.

G7의 표 간 대사가 다른 검문과 성격이 다르다. 소계 항등식은 "그 회사 표 구조가 우리 식과 맞나"에 걸릴 수 있지만, **같은 값이 두 표에 적혀 있는 것은 업종과 무관하게 반드시 일치**한다. §7의 id_label_conflict 사고(발행사채 슬롯에 주식발행초과금 라벨 → 자본 1.4조 둔갑)가 부호·항등식 검사를 전부 통과했던 이유도 여기 있다 — 그 표 안에서는 아귀가 맞았기 때문이다. 자본총계를 자본변동표 기말잔액과 맞대면 그 유형이 드러난다.
...
실제로 준비완료 3개 회사연도에 돌리면 회사당 소계·대사 **18~20건**이 실행되고 위반 0이다. 이 검문이 실제로 잡아낸 것도 있다.

- 첫 실측에서 "자본변동표의 71~88%가 미매핑"이라는 경고가 떴는데, 파보니 **경고 자체가 버그**였다 — 분석이 실제로 쓰는 테이블이 아니라 안 쓰는 표를 재고 있었다(측정 대상을 바로잡으면 0~35%). 이 추적이 더 큰 결함을 끌어냈다: 미매핑 자본거래는 이름이 "기타 중요 계정"이라는 한 덩어리로 뭉개져, 자기주식 매입 582억이 정체 없이 흐르고 있었다. 이름 복원 + 전 수집 데이터 실측 기반 별칭 확충 + 지표 모집단 교정(기초잔액 마커는 거래가 아니므로 제외)으로 표준분류 밖 자본거래를 32~35% → **0~5%**로 줄였다. 잔존 1종(전환사채의 조기상환, 자본총계의 0.05%)은 사전의 기존 선례가 의미상 어긋나 있어 억지 매핑 대신 보류로 남겼다 — 원문 이름으로는 정상 분석된다.
- "사업보고서 서술 추출 0건" 경고가 뜬 회사는 재실행해 보니 추출 자체는 정상(104건)이었다 — 과거의 실패 실행이 "0건 완료"로 위장돼 있던 것이고, 이 위장 경로(실패해도 완료 처리)를 함께 막았다.
- 연도 간 대사는 재고 정정 이력이 있는 회사에서 **재표시 후보 36건**(무형자산 1,761억→1,438억 등)을 자동 포착했다 — 작년 보고서와 올해 보고서가 다른 말을 하는 지점이 검토 큐 재료로 넘어간다.
