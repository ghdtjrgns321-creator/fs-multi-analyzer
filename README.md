...
회사명과 사업연도만 넣으면 5개년 공시를 훑어 **검토 큐**를 만드는 도구

**① 회사명 검색** — 회사명을 입력하면 OpenDART 등록회사에서 상장사를 위로 올려 보여준다. 종목코드와 비상장 태그가 함께 붙는다

<p align="center"><img src="docs/images/ui-flow-1-search.png" width="860" alt="첫 화면 — 회사명을 입력하면 OpenDART 등록회사가 상장사 우선으로 나열된다"></p>

**② 연도 선택과 실행** — 이미 받아둔 연도(raw 보유)와 분석 준비가 끝난 연도(준비완료)를 구분해 보여주고, LLM 전처리와 멀티 에이전트 교차검증을 각각 실행한다

<p align="center"><img src="docs/images/ui-flow-2-prepare.png" width="860" alt="회사 선택 화면 — raw 보유 연도, 준비완료 연도, 분석 사업연도 선택, LLM 전처리, 멀티 에이전트 교차검증 실행 버튼"></p>

**③ 검토 큐** — 재무제표 영역(손익·수익성 / 재무상태 / 현금흐름)별로 계정 카드가 쌓이고, 그 아래에 계정 관계 카드가 붙는다. 한 줄에 계정명·한 줄 결론·지목한 관점이 보여 펼칠 것만 고른다

<p align="center"><img src="docs/images/ui-flow-3-queue.png" width="860" alt="검토 큐 — 손익·수익성, 재무상태, 현금흐름 영역별 계정 카드와 계정 관계 이상 카드 목록"></p>

**④ 카드 한 장** — 펼치면 결론과 원인 경로, 관점별 주장, 근거 수치와 추이 차트, 반대 가능성·확인 질문·다음 절차가 한 화면에 들어온다
...
- 입력과 출력
  - 입력 : 회사명 · 분석 사업연도 — OpenDART 등록회사 118,578개에서 검색
  - 자료 : 분석연도 포함 최근 5개년 · 본표 5종(연결+별도) · 주석 XBRL · 사업보고서 원문
  - 출력 : 검토 큐 카드 — 계정 · 계정 관계 · 회사 전체 3종

- 포지셔닝 — 부정을 확정하지 않는다
  - 분식 자동 적발기가 아니라 **감사인이 볼 이상 변화**를 큐에 올리는 도구
  - 모든 카드에 반대 근거 · 정상 설명 · 확인 질문 · 다음 절차를 함께 붙임

### 2. 문제 인식

#### 감사인이 재무제표를 전부 읽기 어렵다

- 분량
  - 회사 1곳 · 연도 1개 = 본표 5종 × 전 계정 + 주석 수십~수백 항목 + 사업보고서 본문
  - 전기 대비 변화를 보려면 최소 5개년 → 읽을 대상이 회사당 수천 셀
- 축이 서로 다름
  - 본표는 계정 축, 주석은 계정·카테고리 축, 본문 서술은 공시 주제 축
  - "숫자는 그대로인데 주석 문구가 커졌다" 같은 교차는 한 축만 봐서는 안 잡힘
- 어디부터 볼지가 경험에 의존
  - 검토 순서에 정해진 근거가 없어 회사·연도·검토자마다 결과가 달라짐

#### 범용 LLM에 통째로 넣으면 무너지는 것

| 약점       | LLM 단순 투입                      | 본 프로젝트                                     |
| ---------- | ---------------------------------- | ----------------------------------------------- |
| 재현성     | LLM이 직접 수치를 연산 → 숫자 환각 | 코드가 계산 → 숫자 환각 없음, 같은 입력 같은 답 |
| 일관성     | 분석 논리와 방향이 매번 바뀜       | 고정된 다각적 관점(멀티 에이전트)               |
| 전수성     | 토큰 한계로 일부 데이터만 샘플링   | 전 계정·전 연도 결정론 스캔                     |
| 비교가능성 | 회사별·연도별 비교 어려움          | 똑같은 형식의 카드로 결론 제시                  |

### 3. 얻을 수 있는 것

- **읽는 대상이 줄어든다**
  - 전 계정 전수 스캔(코드) → 5개 관점이 지목한 것만 카드로 승격
  - 아스트 2020 실측 : 카드 24장(계정 12 · 관계 5 · 회사 7)
  - 카드 순서 = 반박 판정 → 관점 겹침 수 → 금액 규모 순
...
- **회사와 연도를 나란히 놓을 수 있다**
  - 숫자는 코드가 계산 → 같은 입력이면 같은 수치
  - 관점과 카드 형식이 고정 → 회사가 달라도 같은 자리에서 비교
  - LLM이 만드는 것은 "무엇을 볼지"의 선별과 그 설명뿐
...
처리 7단계 — 단계 이름은 설계 문서의 레이어([PLAN §4](docs/agent/PLAN.md))와 같다

| 단계             | 하는 일                                      | LLM 개입    |
| ---------------- | -------------------------------------------- | ----------- |
| L0 수집          | OpenDART 원본 수집(본표·주석·사업보고서)     | 없음        |
| L1 정규화        | 회사마다 다른 계정을 표준 계정으로 통일      | 없음        |
| L1.5 주석 인덱서 | 주석을 섹션으로 쪼개 계정과 연결             | 없음        |
| 온보딩 게이트    | 파싱 정합성 검증 → 미매핑 보정 · 본문 통독   | 보정·통독만 |
| L2 신호엔진      | 전 계정 전수 스캔 후 계산                    | 없음        |
| L3 역할 에이전트 | 5개 관점이 각자 발견 → 카드 확정 후 외부검증 | 발견·검증   |
| L4 리포트        | 의심 후보를 카드로 묶어 게시                 | 종합·반박   |

온보딩 게이트는 처음 보는 회사가 L2로 들어가기 전 통과해야 하는 관문이다.
LLM이 개입하는 지점은 온보딩 게이트 · L3 · L4 세 곳뿐이고, 숫자를 만드는 단계에는 들어가지 않는다

- **계산은 코드, 발견은 LLM**
  - 증감률·비율·기여도 같은 숫자 연산은 전부 결정론 코드가 수행
  - LLM은 계산된 값을 해석하고 무엇이 의심스러운지만 말함
- **관점은 5개로 고정, 계정은 데이터로 흐른다**
  - 발견 관점 5개 : 숫자 · 주석 · 흐름 · 추세 · 업종
...
### 6. 산출 카드 — 계정 카드와 관계 카드

회사가 달라도 카드 형식은 같다. [1. 프로젝트 소개](#1-프로젝트-소개)의 ④가 계정 카드(아스트 2020 재고자산)이고, 아래는 나머지 두 종류다

<details>
<summary><b>계정 카드 — LG생활건강 2025 당기순이익 (펼치기)</b></summary>

<p align="center"><img src="docs/images/ui-cards-02.png" width="860" alt="당기순이익 카드 — 결론, 원인 경로 6단계, 이상 지점, 남은 확인사항, 검토 포인트, 관점별 주장, 기여도 분해 표, 요인별 기여 폭포 차트, 반박·확인 절차"></p>
<p align="center"><sub>흑자→적자 전환을 세전이익·영업이익·매출총이익 순으로 분해하고, 요인별 기여를 폭포 차트로 보여준다</sub></p>

</details>

<details>
<summary><b>관계 카드 — 매출 ↔ 매출원가 ↔ 재고자산 (아스트 2020) (펼치기)</b></summary>

<p align="center"><img src="docs/images/ui-cards-03.png" width="860" alt="관계 카드 — 매출 급감 대비 재고 증가가 지속됨, 원인 경로 4단계, 이상 지점, 남은 확인사항, 지수화 추이 차트, 반박·확인 절차"></p>
<p align="center"><sub>한 계정이 아니라 계정 사이의 어긋남을 카드로 만든다 — 매출은 -62.3%인데 재고자산은 +10.4%</sub></p>
...
- 계정 카드는 숫자·추세 관점(당해 급변·다년 추세)
- 관계 카드는 흐름 관점(계정 간 괴리)
- 회사 카드는 주석·업종 관점(서술 리스크·동종 대비)
...
> - 본문 통독 : [`reader.py`](src/report/reader.py) · [`layer1.py`](src/report/layer1.py)

데이터 수집·이관 과정의 무결성을 6개 조건으로 검문하고, **모두 충족해야 준비완료**
...
- 본문 통독 — 사업보고서 서술형 본문을 파트별로 읽어 저장
...
| 업종 | 동종사 지표 · 업종 규칙 · 비율 시계열 · 우선순위 상위 5건(비용상 제한)       |
...
━━━ [L4] 카드 생성 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ◆ 카드 순서 — ① 반박이 "정상 설명 우세"로 본 카드는 하단으로
                ② 내부 4관점(숫자·주석·흐름·추세)의 겹침 수 — 업종·외부검증은 집계 제외
...
- 증상 : 내부 3관점(숫자·흐름·추세)이 결국 숫자 관점으로 수렴
