...
| 점검 | [정합성 점검](#3-2-1-정합성-점검)       | 준비 중 유실된 항목 대조 + 대차·합계 검산 → 통과 여부 판정                                                                       |
|      | [데이터 보정](#3-2-2-데이터-보정)       | 표준 사전 미등록 계정 보정 → 등록 시 5개년 재정규화                                                                              |
| 분석 | [신호엔진](#3-3-1-신호엔진)             | 통합 재무 지표 산출 — [계정별 지표](#계정별-지표), [11개 관계사슬](#관계사슬), [15개 재무비율](#재무비율), [변동분해](#변동분해) |
|      | [멀티 에이전트](#3-3-2-멀티-에이전트)   | 각 LLM 독립 실행 — 재무 수치, 주석, 계정 간 관계, 시계열 추세, 업종 비교                                                         |
| 산출 | [리포트 병합](#3-4-1-리포트-병합)       | 인용 수치와 원장 수치 대조 검증, 통과분만 검토 카드로 확정 (외부 근거 검증은 카드 확정 이후)                                     |
...
3. 기술 설명
   ...
   - [3-2. 점검](#3-2-기술-설명--점검) — [정합성 점검](#3-2-1-정합성-점검), [데이터 보정](#3-2-2-데이터-보정)
   - [3-3. 분석](#3-3-기술-설명--분석) — [신호엔진](#3-3-1-신호엔진), [멀티 에이전트](#3-3-2-멀티-에이전트)
   - [3-4. 산출](#3-4-기술-설명--산출) — [리포트 병합](#3-4-1-리포트-병합)
4. [검증](#4-검증)
...
정합성 점검이 지적한 미매핑 계정을 표준 계정에 잇는 단계 — 계정 정규화 단계에서 매핑되지 않은 계정을 LLM으로 보정
...
- 재판정 : 여기서 이름을 이어 붙이면 미매핑 비중이 줄어, 점검을 다시 돌렸을 때 통과 가능
