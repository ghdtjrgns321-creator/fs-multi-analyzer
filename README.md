...
flowchart TD
    A["회사명 · 사업연도"]
    B["코드 — 공시 원문 내려받기"]
    C["코드 — 회사마다 다른 계정명을 표준 계정으로 통일"]
    D["코드 — 옮겨진 수치가 원문과 맞는지 검사"]
    E["LLM — 사업보고서 본문 읽기, 소송·특수관계 등 서술 추출"]
    F["코드 — 5개년 전 계정 계산, 증감·비율·계정 사이 관계"]
    G["LLM — 5개 관점이 각자 의심 항목 선별"]
    H["코드 — 인용 금액을 장부와 대조, 없는 숫자는 탈락"]
    I["검토 카드 — 결론 · 근거 · 반대 가능성 · 다음 절차"]
    A --> B --> C --> D --> E --> F --> G --> H --> I
```
