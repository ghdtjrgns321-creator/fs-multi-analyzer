...
[입력] 회사명 및 사업연도
      ...
      │
      ├─ L1.5 주석 인덱싱    <a href="#3-2-기술-설명--l15-주석-인덱싱">XBRL 주석을 개념 단위로 전개, 주제 28종 분류 후 적재</a>
      │
      ├─ 전처리 게이트       6개 <a href="#3-3-기술-설명--전처리-게이트">정합성 조건 검증</a>, 미매핑 계정 보정 및 본문 서술 중 핵심 텍스트 추출
      │
...
| 구분 | 요약                                | 상세                                                                                                               |
| ---- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| 한계 | 원본 데이터 노이즈 과다             | 회사연도당 평균 1,990건의 주석 데이터 수집 → 58%는 본표 수치와 중복, 15%는 연락처 등 단순 메타 데이터 |
...
> **실제 구현**
> - 온보딩 : [`onboarding_gate.py`](src/normalize/onboarding_gate.py), [`gate_identities.py`](src/normalize/gate_identities.py)
> - 별칭 보정 : [`alias_suggest.py`](src/report/alias_suggest.py)
> - 본문 읽기 : [`reader.py`](src/report/reader.py), [`layer1.py`](src/report/layer1.py)

수집·정규화가 끝난 직후, 신호엔진(L2) 앞에 놓인 검문소다. 이 회사연도의 데이터가 **분석에 쓸 수
있는 상태인지** 판정하고, 못 쓸 상태면 고칠 재료를 만든다. 전처리 실행 한 번에 두 부분이 순서대로
돈다.

```
전처리 실행 1회
  ① 온보딩(결정론)  6개 조건 검문 → 통과 여부 판정
  ② LLM 전처리      본문 읽기 → 별칭 제안 → 고신뢰 자동 등록·5개년 재정규화
```

### 온보딩 — 결정론 게이트

- **판정은 전부 코드** — LLM은 통과 여부에 관여하지 않는다. 뒤의 LLM 전처리는 게이트가 지적한 것을
  고치는 보조 작업이지, 통과를 결정하는 주체가 아니다.
- **왜 막는가** : 정규화가 깨진 채로 5관점 에이전트에 들어가면 틀린 수치 위에서 결론이 나온다.
  잘못된 카드를 만들어 놓고 나중에 걸러내는 것보다, 진입을 거부하고 원인을 보여주는 편이 낫다.
  LLM 호출 비용도 여기서 선차단된다.
- **통과 못 하면** : 분석 진입 자체가 차단된다(`can_enter_analysis`). 어느 조건이 왜 막혔는지
  단계별로 표시되고, 별칭 보정이나 회사 특이사항 등록으로 원인을 고친 뒤 재실행한다.
- **예외 한 가지** : 사업보고서 본문 읽기(LLM)가 실패해도 차단하지 않는다. 경고를 띄우고 사람이
  확인 후 강행한다 — 외부 API 장애로 분석이 영구히 막히는 것을 막되, 서술형 누락은 숨기지 않는다.

여섯 조건을 **모두 충족해야** 다음 단계로 넘어간다.
...
온보딩이 떨어져도 이 단계들은 계속 돈다 — 한 단계 실패가 전체를 막지 않게 설계돼 있고, 실제로
막히는 지점은 전처리가 아니라 분석 진입이다. 별칭이 등록되면 5개년이 재정규화되므로 온보딩을
다시 돌리면 미매핑 비중이 줄어 판정이 바뀔 수 있다.
