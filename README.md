...
┌─ 코드 단위 작업 — 숫자는 전부 여기서 ─────────
│
│   수집 — OpenDART 원본 수집 (정정 전 최초 제출본)
│   정규화 — 공식 사전으로 계정명 통일 · 회사·연도 격리
│   검문 — 데이터 정합성 검사
│   신호 — 전 계정 스캔 · 실제 계산
│
└────────────────────┬──────────────────────────
...
┌─ LLM 분석 — 해석만, 계산 없음 ────────────────
│
│   5관점 병렬 분석 — 숫자 · 주석 · 흐름 · 추세 · 업종
│   사실 대조 — 원본 숫자와 대조, 없는 숫자는 탈락
│   카드·조사·반박 — 의심 · 이유 · 확인사항 · 반대근거 · 다음 절차
│   외부검증 — 출처 대조 · 수치 대사
│
└────────────────────┬──────────────────────────
                     ▼
...
└────────────────────────┬────────────────
                         ▼
┌─ 온보딩 검문 ───────────────────────────
│   빠뜨린 계정 · 자산=부채+자본 · 산수 · 신호가 볼 계정 · 통화
│   실패 → 별칭 3단 분업(코드 후보 → LLM 선택 → 사람 등록 → 사전 반영) → 재검문
└────────────────────────┬────────────────
                         │ 통과
                         ▼
...
━━━ [L2] 신호엔진 — 전수 스캔 · 결정론 · LLM 호출 0회 ━━━━━━━━━━━━━━━━
  ...
  ◆ 계정별 지표   모든 계정에 대해 16종을 계산한다. 순위는 매기지 않는다
      ...
      ── 아래 4개 = 자기 자신을 네 각도로 본 것 ──
      규모 대비 변동폭      |당기−전기| ÷ 자산총계
      추세 강도             방향 일관성 × |당기−최초| ÷ 자산총계
      변동성                표준편차 ÷ |평균|
...
━━━ 조사 — 카드마다 결론을 붙인다. 카드에 따라 경로가 갈린다 ━━━━━━━━
  ...
  ◆ 갈림길   변동 원인이 이미 설명됐나? 아래 중 하나라도 해당하면 "아직"
      ...
      ※ 배제가 아니라 경로 차이다. 모든 카드가 결론을 받는다.
        ...
        │
        ├─ 반박 — 일부러 반대편에 선다
        │    판정 3종 — 정상 설명이 더 그럴듯 / 반반 / 의심이 더 그럴듯
        │    같이 내놓는 것 — 반대 근거 · 정상적인 설명 · 확인 질문 · 다음 절차
        │    ※ 위험도 점수는 안 건드린다 — 판정 표시만 붙인다
        │
        └─ 외부검증
             대상 = 우선순위 상위 3장(무조건) + 조사가 안 끝난 카드 전부(최대 30장)
               └ 상위 카드가 "조사가 잘 끝났다는 이유로" 외부검증에서
                 빠지는 역설을 차단한다
             검색어는 변동 분해가 짚은 주도 요인으로 만든다
               "○○사 2025 영업이익 급감 매출 감소 원인"
             찾아온 기사의 금액을 공시 값과 대조 → 일치 / 불일치 / 대조 불가
               └ 불일치도 지우지 않는다 — "공시와 상이"로 표시
             출처 URL이 없으면 버린다 · 아무것도 못 찾아도 "검증함"으로 기록
                         │
                         ▼
