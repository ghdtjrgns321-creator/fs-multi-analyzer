...
flowchart TD
    ...
    subgraph PREP["준비"]
        direction TB
        A["데이터 수집<br/>OpenDART 연동 — 5개년 재무제표 · 주석 · 사업보고서 원문 적재"]
        B["계정 정규화<br/>기업별 계정명을 표준 사전으로 매핑"]
        C["주석 인덱싱<br/>XBRL 주석 개념 선별 · 주제 28종 분류"]
        D["본문 서술 추출<br/>LLM 기반 사업보고서 분석 · 위험 요소 추출"]
        A --> B --> C --> D
    end

    subgraph GATE["검문"]
        direction TB
        E["정합성 검문<br/>준비 중 유실된 항목 점검 + 대차 · 합계 검산 → 통과 여부 판정"]
        F["데이터 보정<br/>표준 사전 미등록 계정 보정"]
        E --> F
    end

    subgraph ANALYSIS["분석"]
        direction TB
        G["신호엔진<br/>통합 재무 지표 산출 — 계정별 지표 · 11개 관계사슬 · 15개 재무비율 · 변동분해"]
        H["멀티 에이전트<br/>각 LLM 독립 실행 — 재무 수치 · 주석 · 계정 간 관계 · 시계열 추세 · 업종 비교"]
        G --> H
    end

    subgraph OUTPUT["산출"]
        I["리포트 병합<br/>인용 수치와 원장 수치 대조 검증, 통과분만 검토 카드로 확정<br/>외부 근거 검증 에이전트는 카드 확정 이후 수행"]
    end
    ...
    H --> I
    I --> CARD["검토 카드"]
    F -.->|"표준 사전 등록 시 5개년 재정규화"| B
```

> 단계별 상세는 아래 [목차](#목차)의 3. 기술 설명 — 세부 정의는 [표준 계정 사전](#표준-계정-사전-구축) · [계정별 지표](#계정별-지표) · [11개 관계사슬](#관계사슬) · [15개 재무비율](#재무비율) · [변동분해](#변동분해)
...
- 검문과 독립 : 정합성 검문의 판정 결과를 입력으로 쓰지 않으므로 검문보다 먼저 수행 — 추출에 실패해도 다음 단계를 막지 않고 경고만 남긴다(차단 규칙은 [3-2-1](#3-2-1-정합성-검문))
